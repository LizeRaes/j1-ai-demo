# MediFlow - Medical Scheduling System

MediFlow is a web-based medical appointment scheduling system that enables patients to book, manage, and reschedule appointments with healthcare providers. The system provides a user-friendly interface for appointment management and integrates with a centralized helpdesk system to route all user support requests.

## System Overview

MediFlow allows users to:
...
MediFlow integrates with the centralized helpdesk system to ensure all user support requests are properly logged and processed. When users submit help requests through the web interface, the system automatically forwards these requests to the helpdesk ingestion endpoint.
...
- Helpdesk API (receives tickets from MediFlow): `http://localhost:8080/api/intake/incoming-request`
